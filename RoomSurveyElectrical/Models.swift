...
/// Records why a ceiling light exists without changing the established
/// manual/automatic 2D placement model. The property is optional on
/// `CeilingLight` so projects saved by older builds continue to decode.
enum CeilingLightSource: String, Codable, Equatable {
    /// A real light observed and recorded from the post-scan camera view.
    case cameraExisting
    /// A proposed light positioned manually in the 2D editor.
    case planManual
    /// A proposed light generated by the automatic 2D layout tool.
    case planAutomatic
}
...
struct CeilingLight: Codable, Identifiable, Equatable {
    ...
    /// Older projects do not contain `source`; infer their original 2D
    /// meaning from `placementMode` without rewriting or invalidating them.
    var resolvedSource: CeilingLightSource {
        source ?? (placementMode == .automatic ? .planAutomatic : .planManual)
    }

    var isExistingAsBuilt: Bool {
        resolvedSource == .cameraExisting
    }
}
...
struct RoomTakeoffSummary: Identifiable, Equatable {
    ...
    let existingCeilingLightCount: Int
    ...
    init(project: RoomProject) {
        ...
        existingCeilingLightCount = (project.ceilingLights ?? []).filter {
    ...
    var ceilingLightCount: Int {
        existingCeilingLightCount
            + manualCeilingLightCount
            + automaticCeilingLightCount
    }
}
