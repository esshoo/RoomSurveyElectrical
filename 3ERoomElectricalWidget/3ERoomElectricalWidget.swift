...
private enum SharedWidgetConfiguration {
        ...
        guard let directory = sharedDirectory(),
              let data = try? Data(
                contentsOf: directory.appendingPathComponent(
                    SharedWidgetConfiguration.snapshotFileName
                )
              ) else {
            return []
        }
    ...
    private static func sharedDirectory() -> URL? {
        FileManager.default.containerURL(
            forSecurityApplicationGroupIdentifier:
                SharedWidgetConfiguration.appGroupIdentifier
        )?.appendingPathComponent(
            SharedWidgetConfiguration.dataFolderName,
            isDirectory: true
        )
    }
}
        ...
        ZStack {
            ...
            VStack(spacing: 10) {
                ...
                Text("3E Room Electrical")
                    .font(.headline)
                Text("افتح التطبيق لإظهار آخر المشروعات")
                    .font(.caption)
...
struct RecentProjectsHomeWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(
            kind: SharedWidgetConfiguration.widgetKind,
